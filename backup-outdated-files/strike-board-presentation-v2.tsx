...
function StrikeBagMini({ accent = C.amber, size = 1 }) {
  ...
  return (
    <svg width={W} height={H} viewBox="0 0 160 250" style={{ display: "block", filter: `drop-shadow(0 ${12*size}px ${28*size}px rgba(0,0,0,0.9)) drop-shadow(0 2px 8px ${accent}22)` }}>
      <defs>
...
function CoverSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", right: 0, top: 0, width: "45%", height: "100%", background: `linear-gradient(135deg, ${C.gold}05 0%, transparent 70%)`, pointerEvents: "none" }}/>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 64px", position: "relative", zIndex: 1 }}>
        <div style={{ fontFamily: "monospace", fontSize: 10, color: C.gold, opacity: 0.55, letterSpacing: "0.45em", marginBottom: 32 }}>KITS ADVISORY GROUP · CONFIDENTIAL · KAG-JRK-BOARD-001</div>
        <div style={{ marginBottom: 8 }}>
          <span style={{ fontFamily: "'Didot','Bodoni MT','Playfair Display',Georgia,serif", fontSize: "clamp(56px,8vw,100px)", fontWeight: 400, color: C.strike, letterSpacing: "0.18em", lineHeight: 1, display: "block" }}>STRIKE</span>
          <span style={{ fontFamily: "'Didot','Bodoni MT',Georgia,serif", fontSize: "clamp(24px,4vw,44px)", fontWeight: 400, color: C.gold, letterSpacing: "0.18em", display: "block" }}>BITES</span>
        </div>
        <div style={{ width: 80, height: 2, background: `linear-gradient(to right, ${C.gold}, transparent)`, margin: "24px 0" }}/>
        <p style={{ fontSize: "clamp(13px,1.8vw,18px)", color: C.cream, opacity: 0.75, margin: "0 0 10px", letterSpacing: "0.04em", fontFamily: "Georgia,serif" }}>Board Meeting — Initial Advisory Presentation</p>
        <p style={{ fontFamily: "monospace", fontSize: 10, color: C.creamDim, opacity: 0.45, margin: "0 0 40px", letterSpacing: "0.2em" }}>{slide.meta}</p>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          {["Premium Portioned Beef Jerky", "Lebanese Market Entry", "GCC Scale Ready"].map((tag, i) => (
            <span key={i} style={{ fontFamily: "monospace", fontSize: 8, color: C.creamDim, background: `${C.ash}`, border: `1px solid ${C.ember}`, padding: "4px 12px", borderRadius: 2, letterSpacing: "0.1em" }}>{tag}</span>
          ))}
        </div>
      </div>
...
function AgendaSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "44px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4px 48px", alignContent: "center", marginTop: 20 }}>
        {slide.items.map((item, i) => (
          <div key={i} style={{ display: "flex", gap: 16, alignItems: "center", padding: "11px 16px", borderBottom: `1px solid ${C.ash}`, borderLeft: `2px solid ${item.color || C.goldDim}`, background: `${item.color || C.goldDim}05`, marginBottom: 2 }}>
            <span style={{ fontFamily: "monospace", fontSize: 10, color: item.color || C.gold, flexShrink: 0, letterSpacing: "0.05em" }}>{item.n}</span>
            <span style={{ fontSize: 13, color: C.cream, flex: 1, lineHeight: 1.4 }}>{item.text}</span>
            <span style={{ fontFamily: "monospace", fontSize: 8, color: C.creamDim, flexShrink: 0, background: C.charcoal, padding: "2px 8px", borderRadius: 2 }}>{item.tag}</span>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 20, padding: "12px 16px", background: `${C.gold}08`, border: `1px solid ${C.gold}20`, borderRadius: 3 }}>
        <p style={{ margin: 0, fontSize: 12, color: C.gold, fontFamily: "Georgia,serif", fontStyle: "italic" }}>"{slide.note}"</p>
      </div>
    </div>
  );
}
...
function StatsSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "44px 72px" }}>
      <SH slide={slide}/>
      <div style={{ margin: "12px 0 24px", padding: "14px 20px", background: `${C.greenBright}10`, border: `1px solid ${C.greenBright}30`, borderLeft: `4px solid ${C.greenBright}`, borderRadius: 3 }}>
        <p style={{ margin: 0, fontSize: 17, color: C.greenBright, fontFamily: "Georgia,serif", fontStyle: "italic", lineHeight: 1.5 }}>{slide.headline}</p>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, flex: 1, alignContent: "start" }}>
        {slide.stats.map((s, i) => (
          <div key={i} style={{ background: `${s.color}08`, border: `1px solid ${s.color}25`, borderTop: `3px solid ${s.color}`, borderRadius: 4, padding: "22px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ fontFamily: "monospace", fontSize: "clamp(22px,2.5vw,38px)", color: s.color, lineHeight: 1, letterSpacing: "-0.02em" }}>{s.value}</div>
            <div>
              <div style={{ fontSize: 13, color: C.cream, marginBottom: 4, lineHeight: 1.4 }}>{s.label}</div>
              <div style={{ fontFamily: "monospace", fontSize: 9, color: C.creamDim, lineHeight: 1.4 }}>{s.sub}</div>
            </div>
          </div>
        ))}
      </div>
      {slide.footnote && <p style={{ margin: "12px 0 0", fontSize: 11, color: C.creamDim, fontFamily: "monospace", letterSpacing: "0.05em" }}>{slide.footnote}</p>}
    </div>
  );
}
...
function TwoColSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "44px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginTop: 20 }}>
        {[slide.left, slide.right].map((col, ci) => (
          <div key={ci} style={{ background: ci === 0 ? `${C.gold}07` : `${C.greenBright}07`, border: `1px solid ${ci === 0 ? C.gold : C.greenBright}20`, borderTop: `3px solid ${ci === 0 ? C.gold : C.greenBright}`, borderRadius: 4, padding: "22px 24px" }}>
            <div style={{ fontFamily: "monospace", fontSize: 10, color: ci === 0 ? C.gold : C.greenBright, letterSpacing: "0.2em", marginBottom: 18, paddingBottom: 10, borderBottom: `1px solid ${ci === 0 ? C.gold : C.greenBright}20` }}>{col.heading}</div>
            {col.items.map((item, ii) => (
              <div key={ii} style={{ display: "flex", gap: 12, marginBottom: 13, alignItems: "flex-start" }}>
                <span style={{ color: ci === 0 ? C.gold : C.greenBright, fontSize: 9, flexShrink: 0, marginTop: 4 }}>→</span>
                <p style={{ margin: 0, fontSize: 13, color: C.cream, opacity: 0.85, lineHeight: 1.65 }}>{item}</p>
              </div>
            ))}
          </div>
        ))}
...
function SegmentSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "44px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14, marginTop: 20 }}>
        {slide.segments.map((seg, i) => (
          <div key={i} style={{ background: `${seg.color}07`, border: `1px solid ${seg.color}25`, borderTop: `3px solid ${seg.color}`, borderRadius: 4, padding: "20px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <div style={{ fontFamily: "monospace", fontSize: 9, color: seg.color, letterSpacing: "0.2em", marginBottom: 6 }}>{seg.priority}</div>
              <div style={{ fontSize: 16, color: C.cream, fontFamily: "Georgia,serif", marginBottom: 4 }}>{seg.name}</div>
              <div style={{ fontFamily: "monospace", fontSize: 10, color: seg.color }}>{seg.demo}</div>
            </div>
            <div style={{ height: 1, background: `${seg.color}20` }}/>
            <div>
              <div style={{ fontFamily: "monospace", fontSize: 9, color: C.creamDim, marginBottom: 8, letterSpacing: "0.1em" }}>WHAT THEY WANT</div>
              {seg.wants.map((w, wi) => (
                <div key={wi} style={{ display: "flex", gap: 8, marginBottom: 6 }}>
                  <span style={{ color: seg.color, fontSize: 9, flexShrink: 0, marginTop: 2 }}>◈</span>
                  <p style={{ margin: 0, fontSize: 11, color: C.creamDim, lineHeight: 1.5 }}>{w}</p>
                </div>
              ))}
            </div>
            <div style={{ background: `${seg.color}10`, borderRadius: 3, padding: "8px 12px" }}>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: seg.color, marginBottom: 4 }}>REACH THROUGH</div>
              <div style={{ fontSize: 11, color: C.cream }}>{seg.channel}</div>
            </div>
          </div>
        ))}
      </div>
...
function ProductSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <p style={{ fontSize: 14, color: C.gold, fontFamily: "Georgia,serif", margin: "8px 0 18px", fontStyle: "italic" }}>{slide.tagline}</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
        {slide.pillars.map((p, i) => (
          <div key={i} style={{ background: C.charcoal, border: `1px solid ${C.ash}`, borderLeft: `2px solid ${C.gold}50`, borderRadius: 3, padding: "11px 14px" }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 5 }}>
              <span style={{ color: C.gold, fontSize: 10 }}>{p.icon}</span>
              <span style={{ fontFamily: "monospace", fontSize: 8, color: C.gold, letterSpacing: "0.1em" }}>{p.label}</span>
            </div>
            <p style={{ margin: 0, fontSize: 12, color: C.cream, opacity: 0.85, lineHeight: 1.5 }}>{p.value}</p>
          </div>
        ))}
      </div>
      <div style={{ fontFamily: "monospace", fontSize: 9, color: C.creamDim, letterSpacing: "0.2em", marginBottom: 8 }}>PRODUCT LINE ARCHITECTURE — PHASED</div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
        {slide.line.map((l, i) => (
          <div key={i} style={{ background: `${l.color}08`, border: `1px solid ${l.color}30`, borderLeft: `3px solid ${l.color}`, borderRadius: 3, padding: "10px 12px", opacity: i === 0 ? 1 : 0.75 }}>
            <div style={{ fontFamily: "monospace", fontSize: 8, color: l.color, marginBottom: 5, letterSpacing: "0.1em" }}>{l.phase}</div>
            <div style={{ fontSize: 13, color: C.cream, fontFamily: "Georgia,serif", marginBottom: 3 }}>{l.name}</div>
            <div style={{ fontSize: 10, color: l.color, marginBottom: 4 }}>{l.flavor}</div>
            <div style={{ fontFamily: "monospace", fontSize: 8, color: C.creamDim }}>{l.channel}</div>
            {i === 0 && <div style={{ marginTop: 6, fontFamily: "monospace", fontSize: 7, color: l.color, background: `${l.color}15`, padding: "2px 6px", borderRadius: 2, display: "inline-block" }}>LAUNCH</div>}
          </div>
        ))}
      </div>
    </div>
...
// ─── BRAND VISUAL ─────────────────────────────────────────────────────────────
function BrandVisualSlide({ slide }) {
  const accents = [C.amber, C.red, C.greenBright];
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, marginTop: 16 }}>
        {/* Left: bag renders */}
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, letterSpacing: "0.2em", marginBottom: 4 }}>PACKAGING SYSTEM — ALL THREE LAUNCH SKUs</div>
          <div style={{ flex: 1, background: `radial-gradient(ellipse at 50% 40%, ${C.charcoal} 0%, ${C.obsidian} 70%)`, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", gap: 20, padding: "24px 16px" }}>
            {[0, 1, 2].map((i) => (
              <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                <div style={{ transform: `scale(${i === 1 ? 1.08 : 0.9}) translateY(${i === 1 ? -4 : 0}px)` }}>
                  <StrikeBagMini accent={accents[i]} size={0.72}/>
                </div>
                <div style={{ fontFamily: "monospace", fontSize: 7, color: accents[i], letterSpacing: "0.1em", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, letterSpacing: "0.2em", marginBottom: 4 }}>BRAND SYSTEM APPLIED</div>
          {slide.elements.map((el, i) => (
            <div key={i} style={{ background: C.charcoal, border: `1px solid ${el.color}20`, borderLeft: `3px solid ${el.color}`, borderRadius: 3, padding: "10px 14px", display: "flex", gap: 12, alignItems: "flex-start" }}>
              <div style={{ flexShrink: 0, width: 8, height: 8, borderRadius: "50%", background: el.color, marginTop: 4 }}/>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "monospace", fontSize: 8, color: el.color, letterSpacing: "0.1em", marginBottom: 3 }}>{el.label}</div>
                <p style={{ margin: 0, fontSize: 11, color: C.creamDim, lineHeight: 1.5 }}>{el.note}</p>
              </div>
              <span style={{ fontFamily: "monospace", fontSize: 8, color: el.verdict === "CONFIRMED" ? C.greenBright : C.gold, background: `${el.verdict === "CONFIRMED" ? C.greenBright : C.gold}15`, padding: "1px 6px", borderRadius: 2, flexShrink: 0 }}>{el.verdict}</span>
            </div>
          ))}
        </div>
      </div>
...
function BrandScoreSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 16 }}>
        {slide.elements.map((el, i) => (
          <div key={i} style={{ background: C.charcoal, border: `1px solid ${el.verdict === "APPROVED" ? C.greenBright : el.verdict === "RECOMMENDED" ? C.gold : C.amber}15`, borderLeft: `3px solid ${el.verdict === "APPROVED" ? C.greenBright : el.verdict === "RECOMMENDED" ? C.gold : C.amber}`, borderRadius: 3, padding: "12px 14px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, alignItems: "flex-start" }}>
              ...
              <span style={{ fontFamily: "monospace", fontSize: 7, color: el.verdict === "APPROVED" ? C.greenBright : el.verdict === "RECOMMENDED" ? C.gold : C.amber, background: `${el.verdict === "APPROVED" ? C.greenBright : el.verdict === "RECOMMENDED" ? C.gold : C.amber}15`, padding: "1px 6px", borderRadius: 2, flexShrink: 0, marginLeft: 6 }}>{el.verdict}</span>
            </div>
            <div style={{ fontSize: 14, color: C.cream, fontFamily: "Georgia,serif", marginBottom: 5 }}>{el.value}</div>
...
// ─── POWER POINTS ────────────────────────────────────────────────────────────
function PowerPointsSlide({ slide }) {
  const [active, setActive] = useState(0);
  const pp = slide.points[active];
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "auto 1fr", gap: 20, marginTop: 16 }}>
        {/* Left: numbered list */}
        <div style={{ display: "flex", flexDirection: "column", gap: 4, width: 220 }}>
          {slide.points.map((p, i) => (
            <button key={i} onClick={() => setActive(i)} style={{ background: active === i ? `${p.color}18` : "transparent", border: `1px solid ${active === i ? p.color : C.ash}`, borderLeft: `3px solid ${active === i ? p.color : C.ash}`, borderRadius: 3, padding: "10px 14px", cursor: "pointer", textAlign: "left", transition: "all 0.2s" }}>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: p.color, marginBottom: 3, letterSpacing: "0.1em" }}>{p.rank}</div>
              <div style={{ fontSize: 12, color: active === i ? C.cream : C.creamDim, lineHeight: 1.3 }}>{p.title}</div>
            </button>
          ))}
        </div>
        {/* Right: detail */}
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 20px", background: `${pp.color}10`, border: `1px solid ${pp.color}30`, borderRadius: 4 }}>
            <div style={{ fontFamily: "monospace", fontSize: 32, color: pp.color, lineHeight: 1, opacity: 0.4 }}>{pp.rank}</div>
            <div>
              <div style={{ fontSize: 18, color: C.cream, fontFamily: "Georgia,serif" }}>{pp.title}</div>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: pp.color, marginTop: 4, letterSpacing: "0.1em" }}>STRUCTURAL ADVANTAGE</div>
            </div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, flex: 1 }}>
            <div style={{ background: `${C.gold}07`, border: `1px solid ${C.gold}20`, borderTop: `2px solid ${C.gold}`, borderRadius: 3, padding: "14px 16px" }}>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: C.gold, letterSpacing: "0.15em", marginBottom: 8 }}>MIDDLEMAN ADVANTAGE</div>
              <p style={{ margin: 0, fontSize: 12, color: C.cream, lineHeight: 1.7 }}>{pp.middleman}</p>
            </div>
            <div style={{ background: `${C.greenBright}07`, border: `1px solid ${C.greenBright}20`, borderTop: `2px solid ${C.greenBright}`, borderRadius: 3, padding: "14px 16px" }}>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: C.greenBright, letterSpacing: "0.15em", marginBottom: 8 }}>END USER ADVANTAGE</div>
              <p style={{ margin: 0, fontSize: 12, color: C.cream, lineHeight: 1.7 }}>{pp.enduser}</p>
            </div>
          </div>
          <div style={{ background: `${C.red}08`, border: `1px solid ${C.red}20`, borderRadius: 3, padding: "12px 16px" }}>
            <div style={{ fontFamily: "monospace", fontSize: 8, color: C.red, letterSpacing: "0.15em", marginBottom: 6 }}>WHAT COMPETITORS DO INSTEAD</div>
            <p style={{ margin: 0, fontSize: 11, color: C.creamDim, lineHeight: 1.6 }}>{pp.vs}</p>
          </div>
        </div>
      </div>
    </div>
...
function MatrixSlide({ slide }) {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <p style={{ fontSize: 11, color: C.creamDim, fontFamily: "monospace", margin: "4px 0 14px", letterSpacing: "0.1em" }}>{slide.subtitle}</p>
      <div style={{ flex: 1, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr>
              {headers.map((h, i) => (
                <th key={i} style={{ padding: "9px 12px", fontFamily: "monospace", fontSize: 9, color: i === 0 ? C.creamDim : Object.values(hColors)[i-1], letterSpacing: "0.1em", textAlign: "left", borderBottom: `2px solid ${i === 5 ? C.greenBright : C.ash}`, background: i === 5 ? `${C.greenBright}08` : "transparent", whiteSpace: "nowrap" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
      ...
      <div style={{ marginTop: 12, padding: "10px 16px", background: `${C.greenBright}10`, border: `1px solid ${C.greenBright}25`, borderRadius: 3, display: "flex", gap: 10, alignItems: "center" }}>
        <span style={{ color: C.greenBright, fontSize: 16 }}>★</span>
        <p style={{ margin: 0, fontSize: 12, color: C.greenBright, fontFamily: "Georgia,serif" }}>STRIKE wins on all seven criteria. No competitor holds more than three of these advantages simultaneously.</p>
      </div>
    </div>
  );
}
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 16, marginBottom: 16 }}>
        {slide.phases.map((p, i) => (
          <button key={i} onClick={() => setActive(i)} style={{ flex: 1, background: active === i ? `${p.color}18` : C.charcoal, border: `1px solid ${active === i ? p.color : C.ash}`, borderTop: `3px solid ${active === i ? p.color : C.ash}`, borderRadius: 3, padding: "10px 12px", cursor: "pointer", textAlign: "left", transition: "all 0.2s" }}>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div>
          <div style={{ fontFamily: "monospace", fontSize: 9, color: ph.color, letterSpacing: "0.2em", marginBottom: 12 }}>ACTIONS</div>
          {ph.actions.map((a, ai) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ background: `${C.gold}06`, border: `1px solid ${C.gold}15`, borderRadius: 4, padding: "14px 18px", flex: 1 }}>
            <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, letterSpacing: "0.15em", marginBottom: 8 }}>KEY PRINCIPLE</div>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16, marginBottom: 16 }}>
        {slide.tiers.map((tier, i) => (
          <button key={i} onClick={() => setActiveTier(i)} style={{ flex: 1, background: activeTier === i ? `${tier.color}18` : C.charcoal, border: `1px solid ${activeTier === i ? tier.color : C.ash}`, borderRadius: 4, padding: "12px 16px", cursor: "pointer", textAlign: "left", transition: "all 0.2s", position: "relative" }}>
            {tier.recommended && <div style={{ position: "absolute", top: -9, left: "50%", transform: "translateX(-50%)", fontFamily: "monospace", fontSize: 7, color: C.gold, background: C.obsidian, padding: "1px 8px", border: `1px solid ${C.gold}40`, borderRadius: 2, whiteSpace: "nowrap" }}>KITS RECOMMENDED</div>}
            <div style={{ fontFamily: "monospace", fontSize: 8, color: tier.color, letterSpacing: "0.15em", marginBottom: 4 }}>{tier.tier}</div>
            <div style={{ fontSize: 12, color: C.cream, marginBottom: 4 }}>{tier.name}</div>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
            {[{ label: "Breakeven", value: t.breakeven }, { label: "Gross Margin", value: t.margin }, { label: "Rev at M6", value: t.rev_m6 }].map((m, i) => (
              <div key={i} style={{ background: C.charcoal, border: `1px solid ${t.color}20`, borderRadius: 3, padding: "10px 12px" }}>
                <div style={{ fontFamily: "monospace", fontSize: 7, color: C.creamDim, marginBottom: 4 }}>{m.label}</div>
                <div style={{ fontFamily: "monospace", fontSize: 13, color: t.color }}>{m.value}</div>
              </div>
            ))}
          </div>
          {/* Revenue bar chart */}
            ...
            {[
              ...
              <div key={ri} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                  <span style={{ fontFamily: "monospace", fontSize: 9, color: C.creamDim }}>{r.label}</span>
                  <span style={{ fontFamily: "monospace", fontSize: 9, color: t.color }}>{r.text}</span>
                </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "monospace", fontSize: 9, color: C.creamDim, letterSpacing: "0.15em", marginBottom: 8 }}>WHAT'S INCLUDED</div>
...
function BlockersSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <p style={{ fontSize: 11, color: C.creamDim, fontFamily: "monospace", margin: "4px 0 14px", letterSpacing: "0.08em" }}>{slide.subtitle}</p>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
        {slide.blockers.map((b, i) => (
          <div key={i} style={{ background: C.charcoal, border: `1px solid ${b.color}20`, borderLeft: `4px solid ${b.color}`, borderRadius: 3, padding: "10px 16px", display: "flex", gap: 16, alignItems: "flex-start" }}>
            <div style={{ flexShrink: 0, width: 100 }}>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: b.color, background: `${b.color}15`, border: `1px solid ${b.color}30`, padding: "2px 8px", borderRadius: 2, letterSpacing: "0.08em", display: "inline-block", marginBottom: 4 }}>{b.urgency}</div>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: C.creamDim, lineHeight: 1.4 }}>{b.owner}</div>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, color: C.cream, fontFamily: "Georgia,serif", marginBottom: 3 }}>{b.item}</div>
              <p style={{ margin: "0 0 4px", fontSize: 11, color: C.creamDim, lineHeight: 1.5 }}>{b.impact}</p>
              <div style={{ fontFamily: "monospace", fontSize: 9, color: b.color }}>→ {b.action}</div>
            </div>
          </div>
        ))}
      </div>
...
// ─── RISK REGISTER ────────────────────────────────────────────────────────────
function RiskSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16, alignContent: "start" }}>
        {slide.risks.map((r, i) => (
          <div key={i} style={{ background: C.charcoal, border: `1px solid ${r.levelColor}20`, borderLeft: `4px solid ${r.levelColor}`, borderRadius: 3, padding: "12px 16px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
              <div style={{ fontSize: 13, color: C.cream, fontFamily: "Georgia,serif", flex: 1 }}>{r.risk}</div>
              <span style={{ fontFamily: "monospace", fontSize: 8, color: r.levelColor, background: `${r.levelColor}15`, padding: "2px 8px", borderRadius: 2, marginLeft: 10, flexShrink: 0 }}>{r.level}</span>
            </div>
            <p style={{ margin: "0 0 8px", fontSize: 11, color: C.creamDim, lineHeight: 1.5 }}>{r.detail}</p>
            <div style={{ fontFamily: "monospace", fontSize: 9, color: C.greenBright }}>MITIGATION → {r.mitigation}</div>
          </div>
        ))}
      </div>
    </div>
...
function MandateSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
      <SH slide={slide}/>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginTop: 16 }}>
        <div>
          <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, letterSpacing: "0.2em", marginBottom: 12 }}>KITS SCOPE OF AUTHORITY</div>
          {slide.scope.map((area, i) => (
            <div key={i} style={{ marginBottom: 14 }}>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: C.creamDim, letterSpacing: "0.1em", marginBottom: 6, paddingBottom: 4, borderBottom: `1px solid ${C.ash}` }}>{area.area.toUpperCase()}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, letterSpacing: "0.2em", marginBottom: 10 }}>GOVERNANCE MODEL</div>
            {slide.governance.map((g, i) => (
              <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8, padding: "9px 12px", background: C.charcoal, borderRadius: 3, border: `1px solid ${C.ash}` }}>
                <span style={{ color: C.gold, fontSize: 9, flexShrink: 0 }}>◈</span>
          ...
          <div>
            ...
            <p style={{ fontSize: 11, color: C.creamDim, margin: "0 0 10px", fontFamily: "monospace" }}>{slide.fees.note}</p>
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", padding: "36px 72px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", position: "absolute", top: -8, left: 0, right: 0 }}>
          {slide.milestones.map((m, i) => (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", width: `${100/slide.milestones.length}%` }}>
              <div style={{ width: 18, height: 18, borderRadius: "50%", background: m.color, border: `3px solid ${C.obsidian}`, boxShadow: `0 0 0 2px ${m.color}40`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: C.obsidian }}/>
              </div>
              <div style={{ fontFamily: "monospace", fontSize: 8, color: m.color, letterSpacing: "0.1em", marginTop: 6, textAlign: "center", whiteSpace: "nowrap" }}>{m.week}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 8 }}>
        {slide.milestones.map((m, i) => (
          <div key={i} style={{ background: `${m.color}06`, border: `1px solid ${m.color}20`, borderTop: `2px solid ${m.color}`, borderRadius: 3, padding: "12px 10px" }}>
            <div style={{ fontSize: 11, color: C.cream, fontFamily: "Georgia,serif", marginBottom: 8, lineHeight: 1.3 }}>{m.label}</div>
            {m.items.map((item, ii) => (
              <div key={ii} style={{ display: "flex", gap: 6, marginBottom: 5, alignItems: "flex-start" }}>
                <span style={{ color: m.color, fontSize: 8, flexShrink: 0, marginTop: 2 }}>✓</span>
                <p style={{ margin: 0, fontSize: 10, color: C.creamDim, lineHeight: 1.5 }}>{item}</p>
              </div>
            ))}
          </div>
        ))}
      </div>
      <div style={{ marginTop: 12, padding: "10px 16px", background: `${C.greenBright}08`, border: `1px solid ${C.greenBright}20`, borderRadius: 3, display: "flex", gap: 10, alignItems: "center" }}>
        <span style={{ color: C.greenBright, fontSize: 12, flexShrink: 0 }}>◎</span>
        <p style={{ margin: 0, fontSize: 12, color: C.greenBright, fontFamily: "Georgia,serif" }}>From board decision to first sale: <strong>14–18 weeks.</strong> Every week of delay on today's blockers adds a week to this timeline.</p>
      </div>
    </div>
  );
}
...
function ClosingSlide({ slide }) {
  return (
    <div style={{ height: "100%", display: "flex", position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", inset: 0, opacity: 0.03, backgroundImage: `linear-gradient(${C.gold} 1px, transparent 1px), linear-gradient(90deg, ${C.gold} 1px, transparent 1px)`, backgroundSize: "40px 40px" }}/>
      <div style={{ position: "absolute", right: 0, top: 0, width: "40%", height: "100%", background: `linear-gradient(135deg, ${C.gold}05 0%, transparent 70%)` }}/>
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 3, background: `linear-gradient(to bottom, transparent, ${C.gold}, transparent)` }}/>
      {/* Left */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 56px", position: "relative", zIndex: 1 }}>
        <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, opacity: 0.6, letterSpacing: "0.4em", marginBottom: 24 }}>KITS ADVISORY GROUP · DECISION REQUIRED</div>
        <h2 style={{ fontFamily: "'Didot','Bodoni MT','Playfair Display',Georgia,serif", fontSize: "clamp(16px,2.2vw,26px)", fontWeight: 400, color: C.strike, margin: "0 0 28px", lineHeight: 1.5, maxWidth: 520 }}>{slide.title}</h2>
        <div style={{ fontFamily: "monospace", fontSize: 9, color: C.red, letterSpacing: "0.2em", marginBottom: 14 }}>FIVE DECISIONS — REQUIRED TODAY</div>
        {slide.decisions.map((d, i) => (
          <div key={i} style={{ display: "flex", gap: 12, marginBottom: 10, padding: "10px 14px", background: `${C.red}06`, border: `1px solid ${C.red}18`, borderLeft: `3px solid ${C.red}`, borderRadius: 3 }}>
...
function SH({ slide }) {
  return (
    <div style={{ marginBottom: 4, flexShrink: 0 }}>
      {slide.section && <div style={{ fontFamily: "monospace", fontSize: 9, color: C.gold, opacity: 0.55, letterSpacing: "0.35em", marginBottom: 6 }}>{slide.section}</div>}
      <h2 style={{ margin: "0 0 2px", fontFamily: "'Didot','Bodoni MT','Playfair Display',Georgia,serif", fontSize: "clamp(17px,2.3vw,30px)", fontWeight: 400, color: C.cream, lineHeight: 1.2 }}>{slide.title}</h2>
      <div style={{ width: 40, height: 2, background: `linear-gradient(to right, ${C.gold}, transparent)`, marginTop: 8 }}/>
    </div>
  );
}
...
const SLIDES = [
  ...
  },
  {
    ...
    notes: {
      open: "The product is deliberately simple. Poppable beef jerky bites. High protein. Zero sugar. Halal certified. Lebanese made. That is the entire story in one sentence. Everything else — the brand, the packaging, the flavor profile — exists to make that sentence credible and compelling at the point of purchase.",
      emphasis: [
  ...
  },
  {
    ...
    points: [
      ...
      { rank: "06", title: "Lebanese BBQ — The Only Local Flavor", color: C.amber, middleman: "A flavor people recognize and want. Stores carrying Jack Link's Teriyaki sell to a niche. Lebanese BBQ sells to everyone.", enduser: "Cedar smoke, Aleppo heat, pomegranate brightness — this tastes like Lebanon. An emotional ownership no import can replicate.", vs: "All direct competitors use American or British flavor profiles. None have designed for the Lebanese palate. The flavor white space is entirely ours." },
...
function RenderSlide({ slide }) {
  switch (slide.type) {
    ...
    case "segment": return <SegmentSlide slide={slide}/>;
    ...
    case "risks": return <RiskSlide slide={slide}/>;
...
export default function BoardPresentation() {
  ...
  const sectionSlides = SLIDES.reduce((acc, s, i) => {
    if (s.type === "section") acc.push({ ...s, idx: i });
    return acc;
  }, []);
    ...
    <div style={{ minHeight: "100vh", background: C.void, display: "flex", flexDirection: "column", fontFamily: "Georgia, serif", userSelect: "none" }}>
      ...
      <div style={{ background: C.void, borderBottom: `1px solid ${C.ash}`, padding: "7px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, zIndex: 20 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <button onClick={() => setShowNav(!showNav)} style={{ background: showNav ? `${C.gold}15` : "transparent", border: `1px solid ${showNav ? C.gold : C.ash}`, borderRadius: 3, padding: "4px 10px", cursor: "pointer", fontFamily: "monospace", fontSize: 8, color: showNav ? C.gold : C.creamDim, letterSpacing: "0.1em" }}>SLIDES</button>
          <button onClick={() => setShowNotes(!showNotes)} style={{ background: showNotes ? `${C.gold}15` : "transparent", border: `1px solid ${showNotes ? C.gold : C.ash}`, borderRadius: 3, padding: "4px 10px", cursor: "pointer", fontFamily: "monospace", fontSize: 8, color: showNotes ? C.gold : C.creamDim, letterSpacing: "0.1em" }}>NOTES {showNotes ? "▲" : "▼"}</button>
      ...
      {showNav && (
        <div style={{ background: C.obsidian, borderBottom: `1px solid ${C.ash}`, padding: "10px 24px", display: "flex", gap: 4, flexWrap: "wrap", flexShrink: 0, zIndex: 19 }}>
          {SLIDES.map((s, i) => (
            <button key={i} onClick={() => { setCurrent(i); setShowNav(false); }} style={{ background: current === i ? `${C.gold}18` : s.type === "section" ? `${C.ember}` : "transparent", border: `1px solid ${current === i ? C.gold : C.ash}`, borderRadius: 3, padding: "5px 10px", cursor: "pointer", transition: "all 0.15s", minWidth: s.type === "section" ? 70 : 48 }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
        ...
        {showNotes && slide.notes && (
          <div style={{ flex: "0 0 38%", background: "#08070A", borderTop: `2px solid ${C.gold}35`, overflowY: "auto", padding: "16px 32px" }}>
      ...
      <div style={{ background: C.void, borderTop: `1px solid ${C.ash}`, padding: "6px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 20 }}>
          <button onClick={prev} disabled={current === 0} style={{ background: "transparent", border: "none", cursor: current === 0 ? "default" : "pointer", fontFamily: "monospace", fontSize: 8, color: current === 0 ? C.ash : C.creamDim, letterSpacing: "0.1em" }}>← PREV</button>
          <button onClick={next} disabled={current === total - 1} style={{ background: "transparent", border: "none", cursor: current === total - 1 ? "default" : "pointer", fontFamily: "monospace", fontSize: 8, color: current === total - 1 ? C.ash : C.creamDim, letterSpacing: "0.1em" }}>NEXT →</button>
        </div>
        {/* Dot pagination */}
        <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
          {SLIDES.map((s, i) => (
            <button key={i} onClick={() => setCurrent(i)} style={{ width: i === current ? 22 : s.type === "section" ? 8 : 5, height: s.type === "section" ? 5 : 4, borderRadius: 3, background: i === current ? C.gold : s.type === "section" ? C.goldDim : C.ash, border: "none", cursor: "pointer", padding: 0, transition: "all 0.25s ease" }}/>
          ))}
        </div>
